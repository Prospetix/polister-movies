...
function Notes() {
  ...
  function updateNote(id, e) {
    ...
    notes.value = notes.value.map(
      item => (item.id === id ? { ...item, content: save } : item)
    );
  }

  const expand = id => {
    notes.value = notes.value.map(
      item => (item.id === id ? { ...item, expand: true } : item)
    );
  };

  const close = id => {
    notes.value = notes.value.map(
      item => (item.id === id ? { ...item, expand: false } : item)
    );
  };

  const edit = id => {
    notes.value = notes.value.map(
      item => (item.id === id ? { ...item, edit: !item.edit } : item)
    );
  };
          ...
          .map(item =>
            <AnimatePresence>
              ...
              >
                ...
                <h6 style={{color:colors.value[1], userSelect:"none"}} >
                  {item.date}
                </h6>
                <button
